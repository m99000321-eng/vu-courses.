...
import { Eye, EyeOff, GraduationCap, Lock, Mail, ShieldAlert, Sparkles, User } from 'lucide-react'
...
export default function RegisterPage() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.timeline({ defaults: { ease: 'power3.out' } })
        .fromTo('.register-card', { opacity: 0, y: 60, scale: 0.95, rotationX: 10 }, { opacity: 1, y: 0, scale: 1, rotationX: 0, duration: 1.1 })
        .fromTo('.form-item', { opacity: 0, y: 25 }, { opacity: 1, y: 0, duration: 0.7, stagger: 0.08 }, '-=0.5')
        .fromTo('.submit-btn', { opacity: 0, scale: 0.92 }, { opacity: 1, scale: 1, duration: 0.6, ease: 'back.out(1.7)' }, '-=0.3')
        .fromTo('.floating-shape', { opacity: 0, scale: 0 }, { opacity: 0.1, scale: 1, duration: 1.5, stagger: 0.2 }, '-=0.8')
    }, containerRef)
    return () => ctx.revert()
  }, [])
  ...
  useEffect(() => {
    ...
    const move = (e: MouseEvent) => {
      ...
      gsap.to(card, { rotateY: x, rotateX: -y, duration: 0.45, ease: 'power2.out' })
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const target = data.user?.role === 'INSTRUCTOR' ? '/instructor/dashboard' : '/student/dashboard'
          ...
          <div className="p-8 md:p-10 space-y-6">
            <div className="flex flex-col items-center text-center space-y-4">
              <div className="relative">
                <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-brand-purple to-brand-orange flex items-center justify-center shadow-2xl shadow-brand-purple/40 relative overflow-hidden"><div className="absolute inset-0 bg-gradient-to-tr from-white/20 to-transparent" /><GraduationCap className="w-10 h-10 text-white relative z-10" /></div>
              ...
              <div>
                <h1 className="text-3xl font-black text-white tracking-tight mb-2">إنشاء حساب جديد</h1>
                <div className="h-0.5 w-16 mx-auto bg-gradient-to-r from-brand-purple to-brand-orange rounded-full mb-3" />
                <p className="text-sm text-slate-400">ابدأ رحلتك نحو الاحتراف الآن</p>
              </div>
            </div>
            ...
            <form onSubmit={handleRegister} className="space-y-4">
              <div className="form-item">
                ...
                <div className="relative"><input type="text" required minLength={2} maxLength={80} autoComplete="name" placeholder="أدخل اسمك" value={name} onChange={(e) => setName(e.target.value)} className="input-field w-full p-3.5 pr-11 bg-slate-800/50 border border-slate-700/50 rounded-xl text-sm text-white placeholder:text-slate-500 outline-none focus:border-brand-purple focus:ring-2 focus:ring-brand-purple/30 transition-all" /><User className="absolute right-3.5 top-3.5 w-4 h-4 text-slate-500" /></div>
              </div>
              <div className="form-item">
                <label className="block text-xs font-bold text-slate-300 mb-1.5">البريد الإلكتروني</label>
                <div className="relative"><input type="email" required autoComplete="email" placeholder={t('enterEmail')} value={email} onChange={(e) => setEmail(e.target.value)} className="input-field w-full p-3.5 pr-11 bg-slate-800/50 border border-slate-700/50 rounded-xl text-sm text-white placeholder:text-slate-500 outline-none focus:border-brand-purple focus:ring-2 focus:ring-brand-purple/30 transition-all" /><Mail className="absolute right-3.5 top-3.5 w-4 h-4 text-slate-500" /></div>
              </div>
              <div className="form-item">
                <label className="block text-xs font-bold text-slate-300 mb-1.5">كلمة المرور</label>
                <div className="relative"><input type={showPassword ? 'text' : 'password'} required minLength={6} autoComplete="new-password" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} className="input-field w-full p-3.5 pr-11 bg-slate-800/50 border border-slate-700/50 rounded-xl text-sm text-white placeholder:text-slate-500 outline-none focus:border-brand-purple focus:ring-2 focus:ring-brand-purple/30 transition-all" /><Lock className="absolute right-3.5 top-3.5 w-4 h-4 text-slate-500" /><button type="button" onClick={() => setShowPassword((v) => !v)} aria-label="إظهار أو إخفاء كلمة المرور" className="absolute left-3.5 top-3.5 text-slate-500 hover:text-slate-300">{showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}</button></div>
              </div>
              <div className="form-item">
                <label className="block text-xs font-bold text-slate-300 mb-1.5">نوع الحساب</label>
                <select value={role} onChange={(e) => setRole(e.target.value)} className="w-full p-3.5 bg-slate-800/50 border border-slate-700/50 rounded-xl text-sm text-white font-bold outline-none focus:border-brand-purple focus:ring-2 focus:ring-brand-purple/30 appearance-none">
                  <option value="STUDENT" className="bg-slate-800">طالب (Student)</option>
                  <option value="INSTRUCTOR" className="bg-slate-800">محاضر (Instructor)</option>
                </select>
              </div>
              <div className="form-item pt-1">
                <button type="submit" disabled={isLoading} className="submit-btn relative w-full py-3.5 bg-gradient-to-l from-brand-purple to-brand-purple-hover hover:from-brand-purple-hover hover:to-brand-purple text-white text-sm font-bold rounded-xl shadow-lg shadow-brand-purple/30 transition-all duration-300 disabled:opacity-60 disabled:cursor-not-allowed overflow-hidden group">
                  ...
                  <span className="relative z-10 flex items-center justify-center gap-2">{isLoading ? <><Sparkles className="w-4 h-4 animate-pulse" />{t('loading')}</> : <><Sparkles className="w-4 h-4" />إنشاء الحساب الآن</>}</span>
                </button>
              </div>
            </form>
